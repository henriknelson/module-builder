...
MAGISK_MODULE_DEPENDS="libandroid-support, ncurses, readline, libffi, openssl, libbz2, libsqlite, gdbm, ncurses-ui-libs, libcrypt, zlib, openssl, libcrypt"
...
MAGISK_MODULE_EXTRA_CONFIGURE_ARGS+=" --enable-unicode=ucs4 --prefix=/data/python"
...
magisk_step_host_build() {
	...
	$MAGISK_MODULE_SRCDIR/configure --prefix=/data/python
	make Parser/pgen
...
magisk_step_pre_configure() {
	...
	sudo chown builder:builder -R /data/python
...
magisk_step_create_zipscripts() {
	...
	echo "if [ ! -f /data/python/bin/pip2 -a -d $MAGISK_PREFIX/lib/python${_MAJOR_VERSION}/site-packages/pip-*.dist-info ]; then rm -Rf $MAGISK_PREFIX/lib/python${_MAJOR_VERSION}/site-packages/pip-*.dist-info ; fi" >> postinst
	# Setup bin/pip2:
	echo "/data/python/bin/python2 -m ensurepip --upgrade --no-default-pip" >> postinst

	## PRE RM:
	# Avoid running on update
	echo "#!/data/python/bin/sh" > prerm:
	...
	# Cleanup *.pyc files
	echo "find /data/python/lib/python${_MAJOR_VERSION} -depth -name *.pyc -exec rm -rf {} +" >> prerm
	# Remove contents of site-packages/ folder:
	echo "rm -Rf /data/python/lib/python${_MAJOR_VERSION}/site-packages/*" >> prerm
	# Remove pip and easy_install installed by ensurepip in postinst:
	echo "rm -f /data/python/bin/pip2* $MAGISK_PREFIX/bin/easy_install-2*" >> prerm
