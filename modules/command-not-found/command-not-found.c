...
static inline int magisk_min3(unsigned int a, unsigned int b, unsigned int c) {
	return (a < b ? (a < c ? a : c) : (b < c ? b : c));
}

static int magisk_levenshtein_distance(char const* restrict s1, char const* restrict s2) {
	...
	for (unsigned x = 1; x <= s2len; x++)
		for (unsigned y = 1; y <= s1len; y++)
			matrix[x][y] = magisk_min3(matrix[x-1][y] + 1, matrix[x][y-1] + 1, matrix[x-1][y-1] + (s1[y-1] == s2[x-1] ? 0 : 1));
	return matrix[s2len][s1len];
}

int main(int argc, char** argv) {
	if (argc != 2) {
		fprintf(stderr, "usage: magisk-command-not-found <command>\n");
		return 1;
	}
	...
	for (int i = 0; i < num_commands; i++) {
		...
		if (current_line[0] == ' ') { // Binary
			...
			int distance = magisk_levenshtein_distance(command_not_found, binary_name);
