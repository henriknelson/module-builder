magisk_setup_ninja() {
	...
	local NINJA_FOLDER=$MAGISK_COMMON_CACHEDIR/ninja-$NINJA_VERSION
	if [ ! -x "$NINJA_FOLDER/ninja" ]; then
		mkdir -p "$NINJA_FOLDER"
		local NINJA_ZIP_FILE=$MAGISK_MODULE_TMPDIR/ninja-$NINJA_VERSION.zip
		magisk_download https://github.com/ninja-build/ninja/releases/download/v$NINJA_VERSION/ninja-linux.zip \
			"$NINJA_ZIP_FILE" \
